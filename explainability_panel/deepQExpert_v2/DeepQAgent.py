...
class DeepQAgent(AgentWithConverter):
    ...
    def my_act(self, transformed_observation, reward, done=False):
        ...
        """
        # print(self._tmp_obs)
        # print(self._tmp_obs.shape)
        # beg_, end_, dtype_ = self.observation_space.get_indx_extract("rho")
        
        # rho_idx = []
        # idx = 0
        # for el in self._indx_obs:
        #     if el in np.arange(655, 714):
        #         rho_idx.append(idx)
        #     idx += 1
        
        overload = np.any(transformed_observation[0][self._rho_idx] >= 1.)
        # print(overload)
    ...
    def init_obs_extraction(self, observation_space):
        ...
        # Find the rho indices to check the overload 
        rho_idx = []
        idx = 0
        for el in self._indx_obs:
            if el in np.arange(655, 714):
