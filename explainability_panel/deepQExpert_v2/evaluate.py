# python evaluate.py --env_name "l2rpn_case14_sandbox" --load_path="deepq_expert" --name="DeepQExpert" --verbose=True --nb_episode=1 --max_steps=1000
# python evaluate.py --load_path="ai4realnet_small" --name="DeepQExpert" --verbose=True --nb_episode=8 --max_steps=8000
#!/usr/bin/env python3
...
from DeepQSimple import DeepQSimple, DEFAULT_NAME
from DeepQ_NNParam import DeepQ_NNParam
from DeepQ_NN import DeepQ_NN
...
def evaluate(env,
    ...
    res = runner.run(path_save=logs_path,
                     ...
                     max_iter=max_steps,
                     pbar=verbose,
                     env_seeds=np.arange(nb_episode))
...
if __name__ == "__main__":
    ...
    # Create dataset env
    env_name = "../ressources/ai4realnet_small"
    env = make(dataset=env_name, # args.env_name,
               reward_class=L2RPNSandBoxScore,
               other_rewards={
                   "reward": L2RPNReward
               })
    ...
    # try:
    #     # select the chronics with `_2`` at the end for the evaluation
    #     env.chronics_handler.real_data.set_filter(lambda x: re.match(".*(_2)$", x) is not None)
